...
| `routeWalk` | `#64748B` | 경로 지도 도보 선(대중교통 안의 도보는 점선) — #687 |
| `routeBus` | `#0891B2` | 경로 지도 버스 선 — #687 |
| `routeSubway` | `#8B5CF6` | 경로 지도 지하철 선 — #687 |
| `routeCar` | `#3B7BF8` | 경로 지도 자동차 선, 단계 형상이 없는 예전 대중교통 구간 — #687 |
...
Figma는 경로를 파란 점선 하나로만 그린다. 이동수단을 구분하도록 팀 합의(2026-09-18)로 아래 규칙을 더했다. 색만으로 뜻을 전하지 않도록 선 모양·경로 화면 범례·구간 목록의 아이콘과 문구를 함께 쓴다.

| 구간 | 색 | 선 | 밝은 지도(`#F5F5F5`) | 어두운 시트(`darkMap`) |
|---|---|---|---|---|
| 도보 구간(도보 선택·도보 대체) | `routeWalk` | 실선 | 4.36:1 | 3.65:1 |
| 대중교통 안의 도보(처음·환승·마지막) | `routeWalk` | 점선 | 4.36:1 | 3.65:1 |
| 버스 | `routeBus` | 실선 | 3.38:1 | 4.72:1 |
| 지하철 | `routeSubway` | 실선 | 3.88:1 | 4.11:1 |
| 자동차 | `routeCar` | 실선 | 3.58:1 | 4.45:1 |
| 단계 형상이 없는 예전 대중교통 구간 | `routeCar` | 실선 | 3.58:1 | 4.45:1 |

- 버스는 처음 합의한 `#0EA5E9`가 밝은 지도에서 2.54:1로 3:1 기준에 미달해 `#0891B2`로 조정했다.
- 경로 비교의 기존 경로는 그대로 `faint` 실선이며 새 경로 선보다 아래에 그린다.
...
- **예외: 여행 중 화면(`ActiveTravelScreen`)에는 ←를 두지 않는다(2026-09-15, #554).** 하단 탭과 함께 보이는 화면이라 `내 여행` 탭으로 나가며, 진입 경로(하단 `여행 중` 탭, 상세의 `여행 진행 화면으로`, 알림)와 관계없이 같다. 시스템 뒤로 가기는 그대로 동작한다.
...
실시간 데이터를 근거로 안내할 때는 **출처와 갱신 시각**을 함께 표시한다(`기상청 3분 전`). 단, F003 장소 검색·상세는 장소별 provider 배지를 표시하지 않고, 정책상 필수 표시만 둔다: Google 평점·영업정보가 있는 영역의 Google attribution과 아래 공공데이터 출처 표기다.

**공공데이터 출처 표기(필수, 2026-09-16).** 한국관광공사 TourAPI 데이터가 보이는 곳에는 출처 표기(`출처: ⓒ한국관광공사`)를 정책상 필수 표기로 반드시 표시한다(공모전 규정 필수 사항, F003 UI-012, F009 UI-011).

- 위치: TourAPI 데이터가 보이는 목록·정보 영역 단위로 한 줄만 둔다(장소 검색 결과 목록, 장소 상세 정보 영역, 대체 장소 후보 목록, 직접 검색 결과 목록). 장소마다 붙이는 배지로 쓰지 않는다.
- 형식: 텍스트만 쓴다. 한국관광공사 CI/BI 로고 이미지, `TourAPI` 단독 표기, 길픽이 공사 소속인 것처럼 오인될 표현(`한국관광공사 제공 서비스` 등)은 쓰지 않는다.
- 모양: Google attribution과 같은 자리·글자 스타일을 따른다.
- 서비스 단위(2026-09-16 팀 확정): 공모전 규정은 출처 표기를 화면 단위가 아니라 **서비스 단위**로 요구한다. 설정 화면 앱 정보 섹션에 서비스 전체 데이터 출처(`장소 정보` · `출처: ⓒ한국관광공사`)를 **필수로** 두고, 이것으로 서비스 내 출처 표기 요건을 충족한다(F012 FR-009a). 장소 이름만 다시 보여 주는 화면(일정 편집·여행 상세·여행 중·경로·알림·변경 경로 미리보기)에는 추가로 표기하지 않는다.
...
3. **최저선은 Figma보다 위다.** 10절 접근성 최저선(48dp 터치, 색 단독 의미 전달 금지, 아이콘 설명)과 9절 화면 상태 요구, Google attribution·공공데이터 출처 표기(`출처: ⓒ한국관광공사`, 필수) 같은 정책 필수 표시. Figma가 이를 어기면 구현은 이 문서를 따르되 모양을 바꾸지 않는 방식(터치 영역 확장, 문구·아이콘 병기)으로 지킨다.
