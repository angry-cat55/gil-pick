...
| F007 | 위치 기반 감지 | 위치 이벤트 기반 도착·출발 감지와 확인·되돌리기 | F006 | IN_PROGRESS | hs |
| F008 | 여행 변수 감지 | 남은 일정의 혼잡도·날씨·운영시간 평가 | F006 | IN_PROGRESS | ts |
...
- `IN_PROGRESS`: 첫 구현 Issue PR에서 반영 완료했으며 현재 상태다.
- `VERIFY`: T032 파생 로직 테스트와 T033 quickstart 검증이 완료되는 PR에서 반영한다.
