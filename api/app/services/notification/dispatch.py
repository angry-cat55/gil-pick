...
DISPATCH_RETRY_DELAYS = (timedelta(seconds=30), timedelta(minutes=2))
...
class NotificationDispatchService:
    ...
    def __init__(
        self, session: AsyncSession, *, client: FcmClient, now=None, sleep=asyncio.sleep
    ) -> None:
    ...
    async def send_one(self, notification: Notification) -> None:
            ...
            (
                await self.session.scalars(
                    select(DeviceSession).where(
                        DeviceSession.user_id == notification.user_id,
                        DeviceSession.revoked_at.is_(None),
                        DeviceSession.fcm_token.is_not(None),
                    )
                )
            ).all()
        )
        ...
        data = {
            ...
            for key, value in {
                ...
                "tripDayId": notification.trip_day_id,
                "itemId": notification.item_id,
                "detectionId": notification.detection_id,
        ...
        for device in sessions:
            ...
            for attempt in range(3):
                try:
                    ...
                    result = (
                        FcmSendResult.RETRYABLE
                        if exc.retryable
                        else FcmSendResult.FATAL
                    )
        ...
        now = self.now()
        successes = summary[FcmSendResult.OK.value]
        retryable = summary[FcmSendResult.RETRYABLE.value]
        failures = (
            summary[FcmSendResult.INVALID_TOKEN.value]
            + summary[FcmSendResult.RETRYABLE.value]
            + summary[FcmSendResult.FATAL.value]
        )
        if not sessions:
            notification.delivery_status = "NO_DEVICE"
            notification.next_attempt_at = None
            event = "notification_delivery_no_device"
        elif successes:
        ...
        elif retryable and notification.delivery_attempts < MAX_DISPATCH_ATTEMPTS:
            notification.delivery_status = "PENDING"
            notification.next_attempt_at = (
                now + DISPATCH_RETRY_DELAYS[notification.delivery_attempts - 1]
            )
        ...
        if (
            getattr(getattr(self.client, "settings", None), "fcm_enabled", True)
            is False
        ):
            event = "fcm_skipped"
        logger.log(
            logging.ERROR if notification.delivery_status == "FAILED" else logging.INFO,
            {
                "event": event,
                "request_id": request_id_context.get() or str(uuid.uuid4()),
                "type": notification.type,
                "target_id": str(
                    notification.detection_id or notification.transition_id
                ),
            ...
            (
                await self.session.scalars(
                    select(ProgressTransition).where(
                        ...
                        cast(
                            ProgressTransition.response_snapshot[
                                "nextPromptAt"
                            ].as_string(),
