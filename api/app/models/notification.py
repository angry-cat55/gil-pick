...
class Notification(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "type IN ('PLACE_CHANGE_SUGGESTION','ARRIVAL_CHECK','DEPARTURE_CHECK','ARRIVAL_AUTO_CONFIRMED','DEPARTURE_AUTO_CONFIRMED')",
        ...
        CheckConstraint(
            "delivery_attempts >= 0", name="ck_notifications_delivery_attempts"
        ),
    ...
    trip_day_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("trip_days.trip_day_id", ondelete="SET NULL")
    )
    item_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("itinerary_items.item_id", ondelete="SET NULL")
    )
    ...
    transition_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("progress_transitions.transition_id", ondelete="SET NULL")
    )
