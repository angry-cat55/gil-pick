...
**Issue #697 보완 (2026-09-18)**: #686 구현은 Kakao 단계 연속성을 판정하는 3m 기준을 TMAP 보행 응답 endpoint에도 적용했다. 기존 fixture는 TMAP이 요청 좌표를 정확히 반환해 도로망 스냅을 재현하지 못했다. 요청점과 TMAP 형상 endpoint가 각각 30m 이내이면 정상적인 접근 스냅으로 허용하고 실제 형상의 끝점만 요청 좌표에 맞춘다. 30m를 넘는 이탈은 `ROUTE_INVALID_RESULT`로 유지한다. 이 값은 도보 접근 연결만 허용하는 제한값이며 실제 운영 표본으로 재검증해야 한다.
