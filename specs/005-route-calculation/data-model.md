...
- `steps`: 대중교통 상세 단계의 `type`(`WALK`·`BUS`·`SUBWAY`), `durationSeconds`, `distanceMeters`, nullable `boardingName`·`alightingName`·`lineName`·`stopCount`·`geometry`. 신규 Kakao 계산은 모든 단계에 GeoJSON `LineString`을 저장하고 3m 초과 보행 공백은 TMAP 형상으로 보완한다. WALK·CAR와 기존 `steps` 누락 JSONB는 빈 목록, 기존 단계 형상 누락은 `geometry: null`로 조회한다. 기존 `route_payload` JSONB를 확장하므로 DB migration은 없다.
