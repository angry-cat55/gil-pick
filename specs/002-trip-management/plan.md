...
- 2026-09-16: 기간 축소 확인 안내에 날짜별 삭제 일정 수(FR-013a, #589, BE #588)를 추가하고, 사진만 실패한 저장의 재시도 규칙(FR-019b)을 정리했다. 재시도는 이름·기간이 그대로면 수정 요청 없이 이미지 업로드만 보낸다. 빈 수정 요청도 `version`을 확인하므로 사진과 무관한 오류가 뜰 수 있어서다.
...
| II. 계약 우선 SDD와 문서 동기화 | `docs/design/api-spec.md`(TRIP-001~005)·`docs/design/er-schema.md`(`trips`)를 기준 계약으로 사용했다. 완료 상태의 기간 수정 잠금, 기간 중복 금지와 상태 무관 삭제 정책을 관련 명세와 API 계약에 동기화했다. | PASS |
| III. 상태 변경의 일관성·멱등성·추적 가능성 | 여행 생성은 `Idempotency-Key`로 멱등 처리하고(FR-003), 수정은 `version` 낙관적 동시성으로 충돌을 감지하며(FR-011a), 삭제는 반복 요청에도 동일한 결과를 반환한다(FR-016). 사용자별 활성 여행 기간은 DB exclusion constraint로 동시 요청까지 중복을 막는다(FR-002a, FR-002b). | PASS |
...
- `data-model.md`는 `trips` 단일 entity와 검증 규칙·인덱스·기간 exclusion constraint를 정의하며 F004 범위(`trip_days`, `itinerary_items`)를 침범하지 않는다.
- `contracts/trips.openapi.yaml`은 공통 envelope, `Idempotency-Key`, `version` 기반 `409 VERSION_CONFLICT`, 완료 상태 기간 수정 잠금 `409 TRIP_LOCKED`, 기간 중복 `409 TRIP_PERIOD_CONFLICT`, 상태 무관 삭제를 명시한다.
- `quickstart.md`는 정상 흐름뿐 아니라 소유권 위반, 기간 경계·동시 요청 충돌, 버전 충돌, 완료 상태 기간 수정 잠금, 상태 무관 삭제 멱등성, 삭제 확인 흐름을 검증한다.
...
│   └── 012_prevent_overlapping_trip_periods.py  # 활성 여행 기간 exclusion constraint
└── tests/
...
**Structure Decision**: F001과 동일하게 `api/`, `android/` 두 디렉터리를 유지한다. Backend는 `trips.py`를 얇은 transport 계층으로, `services/trip.py`가 트랜잭션 경계(생성 멱등성, 버전 검증, 완료 상태 기간 수정 잠금, 기간 충돌 오류 매핑)를 소유하는 F001 `services/auth.py`와 동일한 패턴을 따른다. 동시 요청까지 사용자별 기간 비중복을 보장하는 DB exclusion constraint와 기존 데이터 사전 검사는 별도 Alembic migration이 소유한다. Android는 `com.gilpick.auth`와 분리된 `com.gilpick.trip` 패키지를 새로 만들고, `MainActivity`의 인증 성공 이후 진입점을 F001이 임시로 둔 `AuthenticatedHomeScreen`에서 `TripListScreen`으로 교체한다. `trip_days`/`itinerary_items`, 지도 표시, 이동수단은 F004~F005 범위이므로 이번 구조에 포함하지 않는다.
...
결정 근거와 대안은 [research.md](research.md)에 기록했다. 기술 스택 관련 `NEEDS CLARIFICATION`은 없다. 다만 "F002가 `trips`만 생성하고 `days[]`를 생략한다"는 결정은 Backend 담당자 확인이 필요한 항목으로 표시해 두었다(research.md 6절).
...
`btree_gist` 확장과 exclusion constraint는 동시 요청에서도 기간 비중복을 보장하기 위한 최소 DB 무결성 장치다. 애플리케이션 조회만 사용하는 더 단순한 구현은 경쟁 상태를 허용하므로 constitution III을 충족하지 못한다. Migration은 기존 충돌 데이터를 자동 수정하지 않고 실패시켜 사용자 데이터 통제권을 보존한다. `trip_days`/`itinerary_items`를 F002에서 미리 만들지 않기로 한 결정에 따라, 기간 축소 시 "삭제될 일정 수" 안내(FR-012)는 F002 구현 시점에는 실제 일정 데이터 없이 항상 0건으로 동작한다. 이 잔여 범위는 F004(일정 구성) plan에서 `trip_days`/`itinerary_items`를 도입하며 함께 채운다.
