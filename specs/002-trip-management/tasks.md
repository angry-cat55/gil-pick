...
**담당자**: 모든 task는 담당 `미정`으로 기록한다. AGENTS.md에 따라 팀 합의 전에는 AI가 임의로 배정하지 않는다. `speckit-taskstoissues` 변환 전에 Backend(jh/ts)·Frontend Android(jy/hs) 담당자를 합의해 반영해야 한다.
...
- Backend 성격 task(T001~T005, T007~T008, T010~T012, T015~T016, T018~T019, T023, T025~T026, T030~T032, T034~T035, T039~T042)는 Backend 담당자가 맡는다.
- Frontend Android 성격 task(T006, T009, T013~T014, T017, T020~T022, T024, T027~T029, T033, T036~T038, T043~T044)는 Frontend Android 담당자가 맡는다.
- Phase 8(T045~T047)은 통합 검증으로 Backend·Frontend가 함께 확인한다.
- 실제 이니셜 배정은 `speckit-taskstoissues` 전에 팀 합의로 확정한다(AGENTS.md 6절).
