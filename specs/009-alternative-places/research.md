...
- **재사용**: 지도는 `RouteMap`의 `circleMarker`·`pillMarker`·NaverMap 초기화 코드를 `internal`로 열어 `AlternativeMap`이 쓴다(경로선 없음, 기존 장소 = 경고 pill, 후보 = 순위 원형). ~~직접 검색 화면은 `PlaceSearchScreen`의 `PlaceRow`·`EmptyState`를 `internal`로 열고 행 아래 거리·방문 가능 표시 slot을 추가해 `AlternativeSearchScreen`이 재사용한다(카테고리 칩·지역 필터 없음, 검색어만).~~ **2026-09-13 폐기(#450)**: 직접 검색 화면은 Figma `MapSearchScreen` 기준 지도형(지도 + 번호 마커 + 카테고리 칩 + 결과 시트 + `선택` 버튼)으로 바꾼다. 지도는 위 `RouteMap` helper 재사용을 그대로 쓸 수 있다. 2026-09-14: FR-019를 "대체 대상 장소 좌표 기준 2km 이내"로 바꿨다. 반경 필터는 R1과 같은 이유(키워드 검색은 반경 미지원, 페이지 단위 응답)로 화면이 아니라 서버에서 적용해야 한다. 카테고리 필터·혼잡/마감 값과 함께 검색 API 변경을 백엔드와 조율 중이며, 앱은 API 반영 전까지 해당 UI를 조건부로 숨긴다(plan.md "변경 기록").
...
- **배너**: `ProgressViewModel`에 `AlternativeRepository`(nullable, 기본 null) 주입 → `load()`에서 DETECT-001(`status=ACTIVE`)을 병렬 조회, 실패는 배너 숨김으로 격리. `Content.activeDetections`와 `bannerDetection`(오늘·당일 미완료일 때 ETA 가장 이른 것 하나, Figma 배너는 단일 항목). `ActiveTravelScreen(onOpenAlternatives: (detectionId) -> Unit)`.
