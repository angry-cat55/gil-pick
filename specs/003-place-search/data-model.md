...
F003은 영구 entity나 DB migration을 만들지 않는다. 아래 모델은 TourAPI 응답을 길픽 API와 Android UI로 전달하는 transient read model이다.
...
|---|---|---:|---|
| `place_id` | string | 아니오 | `tourapi:{contentId}` 형식의 안정적인 ID |
| `source` | literal `TOUR_API` | 아니오 | 외부 원천 식별 |
| `source_place_id` | string | 아니오 | TourAPI `contentId` |
| `name` | string | 아니오 | 빈 값이면 해당 provider item 제외 |
| `category` | `PlaceCategory` | 아니오 | 내부 mapping 결과 |
| `tour_api_category` | `TourApiCategory` | 아니오 | 원본 code 보존 |
...
`open_now`, `closes_at`, Google rating·review는 F003 모델에 없다.
...
| `page_no` | 다음 TourAPI page number |
...
- TourAPI credential, provider 원문 오류와 응답 body는 DB, log, cursor, Android 저장소에 남기지 않는다.
