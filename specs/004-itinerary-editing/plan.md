...
**차이(구현 전 결정 필요)**: Figma 수정으로 `ScheduleEditScreen`의 `{장소명} 체류 시간` 대화상자(`−`·`+`, 60·90·120 빠른 선택, `취소`·`적용`)가 코드에서 제거됐다. 이전 사본에서도 이 대화상자를 여는 요소는 없었고 모양만 정의돼 있었다. spec UI-003·FR-004는 이 대화상자를 요구하므로, 구현은 이전 사본(2026-09-04, git 이력)의 대화상자 모양과 F003 `AddToScheduleSheet` stepper 규칙을 따르되, Figma에 체류 시간 대화상자와 그 진입점(행의 `{시각} · {N}분` 탭)을 추가해 정본을 맞추는 것이 남아 있다.
