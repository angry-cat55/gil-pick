...
- **Decision**: `NotificationService`가 유형별 `title`·`body`를 서버에서 만든다.
  - 장소 변경 제안: `title="{장소명} 변경을 추천해요"`(2026-09-16 #585 — 고정 문구 "다음 장소 변경을 추천해요"는 3·4번째 장소에도 "다음 장소"로 읽혀 부정확했다), `body`는 감지 `reason`(DETECT-002가 이미 쓰는 문구, 예 "도착 시각에 영업이 어렵거나 곧 문을 닫아요")에 장소명을 붙인다. 종합 위험 점수 50점 이상이거나 운영시간 방문 불가일 때만 생성한다(FR-031).
