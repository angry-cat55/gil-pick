...
- 시작 흐름: 권한 없음 → `첫 장소로 이동하기` 잠김, 누르면 `위치 기반 서비스 승인 후, 사용해주세요` 안내 → `위치 권한 허용하기`로 권한 안내·시스템 권한 요청(거부하거나 `나중에 하기`면 시작하지 않고 시트로 복귀). `첫 장소에서 시작하기`는 권한 없이 시작 성공. 허용 시 잠금 해제 후 1회 위치 취득(10초 timeout, 실패하면 위치 없이 시작).
...
| 권한 거부 시작 (T017·T018, #731 이전 동작. 이후 `첫 장소로 이동하기`는 권한이 없으면 잠김) | `pm revoke` + `user-fixed` 상태에서 시작 → 시스템 권한 화면이 거부로 즉시 닫힘 → 위치 null로 start 200 → 진행 화면. 서버 `startLocation=null`, 카드 `이동 정보 없음` | 통과 |
...
| 시작(권한 거부, #731 이전 동작) | `pm revoke` + `user-fixed` → 시스템 권한 화면이 거부로 닫힘 → 위치 null로 start 200(클릭부터 9.1초, 대부분 emulator의 권한 화면 왕복). 카드 `이동 정보 없음`, 서버 `startLocation=null` | 통과 |
