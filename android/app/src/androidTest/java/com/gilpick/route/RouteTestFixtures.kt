...
/** route androidTest가 공유하는 fixture. unit test의 `RouteFixtures.kt`와 같은 값이다. */
...
/** 도보(TMAP)·대중교통(ODsay) 두 구간, 세 장소의 READY 경로. */
...
/** 계약 형식의 `RouteEnvelope` JSON. MockWebServer 응답에 쓴다. */
internal fun routeEnvelopeJson(routeStatus: String, route: String = "null", failure: String = "null", scheduleVersion: Int = 3) = """
    {"success": true, "data": {
...
internal fun readyRouteJson(scheduleVersion: Int = 3) = """
    {
      "routeId": "$ROUTE_ID", "scheduleVersion": $scheduleVersion,
      "totalDurationSeconds": 1500, "totalDistanceMeters": 4200,
      "markers": [
      ...
      "segments": [
        {"sequence": 1, "fromItemId": "$ITEM_A", "toItemId": "$ITEM_B", "transportMode": "WALK", "provider": "TMAP",
         "durationSeconds": 600, "distanceMeters": 800,
         "geometry": {"type": "LineString", "coordinates": [[126.977, 37.5796], [126.98, 37.581], [126.9831, 37.5826]]},
         "providerAttribution": "$TMAP_ATTRIBUTION"},
        {"sequence": 2, "fromItemId": "$ITEM_B", "toItemId": "$ITEM_C", "transportMode": "TRANSIT", "provider": "ODSAY",
         "durationSeconds": 900, "distanceMeters": 3400,
